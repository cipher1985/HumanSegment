...
QtOnnxPPHumanSegment::QtOnnxPPHumanSegment(QObject *parent, const QString &modelFile,
    int modelW, int modelH,
    int opThreads) : QtOnnxPPHumanSegment(modelFile,
        modelW, modelH, opThreads, parent) {}

QtOnnxPPHumanSegment::QtOnnxPPHumanSegment(const QString &modelFile,
    int modelW, int modelH,
    int opThreads, QObject *parent) : QObject{parent},
    m_env(ORT_LOGGING_LEVEL_ERROR, "pphumansegment")
{
...
QImage QtOnnxPPHumanSegment::segmentImage(QImage imgBgra, QImage *retMask)
{
    QImage ret;
    if (imgBgra.isNull())
        return ret;
...
QImage QtOnnxPPHumanSegment::generateMatting(std::vector<Ort::Value> &outputTensors)
{
    //获得通道数据
    ...
    float* p0 = alphaValue.GetTensorMutableData<float>();
    float* p1 = p0 + w * h;
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
...
std::vector<Ort::Value> QtOnnxPPHumanSegment::transformImageData(QImage imgBgr)
{
    //获得缩放图像
...
std::vector<float> QtOnnxPPHumanSegment::normalizeImageData(QImage img)
{
    // 定义图像的颜色通道数量
